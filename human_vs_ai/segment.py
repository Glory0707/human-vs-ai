...
# 链接/图片内容上限：无界 [^\]]* 在大量未闭合 "[" 上是 O(n²)
_RE_IMG = re.compile(r"!\[([^\]]{0,300})\]\(([^)]{0,500})\)")
_RE_LINK = re.compile(r"\[([^\]]{0,300})\]\(([^)]{0,500})\)")
_RE_URL = re.compile(r"(?:https?://|www\.)[^\s，。；！？、）)】」』]+", re.I)
# 邮箱：显式 ASCII 类 + RFC 5321 长度上限（本地部 64/域标签 63）。
# 两点教训：Python \w 认 CJK 而 JS 不认（同正则两端行为漂移）；
# 无界 [\w.+-]+@ 在长 CJK 串上是 O(n²) 灾难性回溯（实测 4 万字 12.7 秒）
_RE_EMAIL = re.compile(
    r"[A-Za-z0-9._%+-]{1,64}@[A-Za-z0-9-]{1,63}(?:\.[A-Za-z0-9._-]{1,63})+")
# 只清内容含中文的星号强调（*斜体*/**粗体**），避免误伤 3*5 这类算式。
# 内容上限 49+CJK+49 字：无上限的 [^*]*? 在"单个未闭合星号+长文"上
# 是 O(n²) 灾难性回溯（实测 4 万字 51 秒）；真实强调跨度远小于 49 字
_RE_EMPH = re.compile(r"\*{1,3}(?!\s)([^*]{0,49}?[一-鿿][^*]{0,49}?)(?<!\s)\*{1,3}")
