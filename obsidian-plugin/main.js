...
function renderReportHtml(profile, result) {
  const parts = [];
  parts.push(`<div class="stats">${sealHtml(result.score)}${result.score_note ? `<div class="row score-note">AI 味指数 —（${esc(result.score_note)}）</div>` : ""}${statsRows(result.stats).map(r => `<div class="row">${esc(r)}</div>`).join("")}</div>`);
