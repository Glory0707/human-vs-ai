...
- **单元测试**：93 项（切分/统计/引擎/边界/报告与文案/评分/口味与改写/端到端区分度）；私库回归 5 项无 corpus_private/ 时自动跳过
...
- **双引擎一致性**：27 段语料 + 12 条改写探针 × 4 profile = **156 项**逐字段 diff 全绿；含 emoji 码点/行分隔符全集/孤立低代理/闭引号吸收/未闭合围栏/双竖线表格/引号不配对/邮箱/括号洪水等对抗探针
