...
> 当前版本见根目录 VERSION。已实现能力见 [README](../README.md)，本文只保留定位、原则、取舍与未竟事项。
...
- **P2**：general profile 当代语料校准（公开渠道反爬与条款风险高，优先本地生成+用户自愿提交的匿名样本，评估见 plan.md M5）· 公文 AI 真实样本扩充（当前为构造样本，有自我偏差）· 评分按长度分档拟合（academic 档短文分数摆放，见 §7）· Obsidian 插件（engine.js 直接复用）
...
- **单元测试**：90 项（切分 12 / 统计 7 / 引擎 10 / 边界 9 / 报告与文案 7 / 端到端区分度 4 / 综合评分 7 / 口味与改写 34）；私库回归 5 项无 corpus_private/ 时自动跳过
...
- **双引擎一致性**：22 段语料 + 12 条改写探针 × 4 profile = **136 项**逐字段 diff 全绿；含 emoji 码点/未闭合围栏/双竖线表格/引号不配对/邮箱/括号洪水等对抗探针
...
- 评分的长度语义：字级 2-gram TTR 在数百字以上趋饱和（≈0.92），polished 长文指数偏高——长档（≥600 字）分档系数已上线（v0.12.0，长档 holdout 0.975），但校准人群仍是摘要级语料，真实长文真人样本待积累；毕设实测（3.6 万字，指数 92→99）为该边界实例
