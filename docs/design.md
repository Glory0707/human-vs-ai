...
- **单元测试**：88 项（切分 12 / 统计 7 / 引擎 10 / 边界 9 / 报告与文案 7 / 端到端区分度 3 / 综合评分 6 / 口味与改写 34）；私库回归 5 项无 corpus_private/ 时自动跳过
...
- 评分的短文摆放：全文统计在几百字级别趋饱和（2-gram TTR≈0.95），academic 档个别短真人样本会落到高档——按长度分档拟合在 P2（general 已用类平衡矫正先验）
