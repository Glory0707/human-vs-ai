...
PROBE_TEXTS = [
    ...
    # 文种域外（ood.detect_genre 双端对拍）：official 场景出 kinds，
...
REWRITE_PROBES = [
    ...
    # 行中收束词（多句段落）：旧实现只剥行首 → 候选与原文等值（视觉走查实修）
    "首先，本文制备了系列的钙钛矿样品。其次，我们对样品进行了全面的表征分析。最后，系统地测试了光电性能。综上所述，本文开展了一系列深入而有效的研究工作。",
    "不得不说。",                        # 只剩收束词：退化行 → 整句删方向
    # 多句段落命中 T2 抒情升华：截半句候选会毁掉整段 → 必须降级为方向（视觉走查实修）
    "在当今快速发展的时代背景下，能源问题日益凸显。与此同时，传统的硅基太阳能电池逐渐接近其理论效率极限。因此，开发新型低成本的光伏技术显得尤为重要。此外，钙钛矿太阳能电池作为一种新兴的光伏技术，扮演着越来越重要的角色，有望在未来的能源格局中发挥关键作用。",
]
