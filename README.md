...
**不想装命令行？** 双击 [web/index.html](web/index.html)——单文件网页版，浏览器打开即用，粘贴即析，同样纯本地（无后端、无网络请求、可离线）。规则与命令行版完全一致，由双引擎一致性测试守护（`python tools/check_web_consistency.py`，22 段分析语料 + 12 条改写探针 × 4 profile = 136 项逐字段对齐，含列表/表格/裸链接/全角数字/emoji 码点/正则洪水探针）；改了规则后用 `python tools/build_web.py` 重新生成。输入即析（长文自动放宽防抖），命中词在原句里高亮，跟随系统暗色模式，报告可一键复制为 Markdown，改写建议按删/改/留过滤。
