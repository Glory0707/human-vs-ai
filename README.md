...
- **报告结构**：指数+构成 → 统计摘要 → 逐句发现（同句多规则聚合，不重复贴原句；命中词高亮）→ 弱命中（长文只列前 12 处）→ 一行免责
- **三种格式同一份内容**：终端（ANSI 彩色）/ Markdown / JSON（事实源永不截断）

### 实测区分度（校准语料，非宣传数字）

| 场景 | 校准语料（过 8 句门槛） | 留出 AUROC | 样本外体检 |
|---|---|---|---|
| academic | C-ReD paper 真人 80 vs 四模型 320（词表 **0.888** · CV **0.799**） | **0.974**（全量长档 0.975） | — |
| general | 知乎真实长回答 110 vs gen2026 当季 69 | **0.935**（旧 HC3 系数同期仅 0.602） | **0.923**（豆瓣/果壳真人 vs 未参拟合四模型，换模型+换渠道 PASS） |
| official | 真人事务公文 71 vs 当季公文 70 | **0.923** | 事务文种五省切片 **0.888**；印发/批复文种域外抑制（渠道指纹实证） |
| essay | C-ReD composition 真人 1070 vs 7 模型 7544 | **0.953** | — |
| news | C-ReD news 真人 1413 vs 7 模型 15112 | **0.948** | — |

按场景的关键单指标：句长 CV 是对四个模型（含最难检的推理模型 deepseek-r1）一致有效的唯一指标；真人公文误报 0/15（官方词表组合覆盖 9/9 当季模型）；HC3 问答上学术词表失效（0.476）、TTR 方向随文体反转——profile 分治的实证；三连排比跨代稳定（HC3 +0.22、C-ReD QA +0.17、gen2026 83% 命中）；LLM 定向洗稿削掉词表命中 89% 后仍 93.3% 超阈值（统计底盘扛住规避）。CV 阈值按长度三档（<300 字 0.30 / <600 字 0.33 / 更长 0.37）。general 的互动尾巴、万能开场是当代特征，2023 语料测不到——诚实标注，不造数字。完整校准表、逐规则区分度与被证伪删除的规则见 [docs/rules.md](docs/rules.md)。
...
```bash
python -m pytest tests/ -q              # 202 项单元+边界+评分+口味+格式+多文体+域外+模糊回归（私库层缺语料自动跳过）
python tools/check_web_consistency.py   # Python/JS 双引擎一致性 427 项 × 7 场景（含清理稿对拍，需 node）
