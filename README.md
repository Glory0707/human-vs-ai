...
- **双场景规则库**：`academic`（论文摘要/正文/实验报告，C-ReD 校准）与 `general`（知乎/公众号/科普/问答，HC3 校准）。实测证明词表规则的区分度强依赖文体（学术套话在问答语料上区分度≈0；三连排比在学术摘要反向、在问答 +0.22），所以词表按 profile 分治，统计底盘共用
...
句长 CV 是对四个模型（含最难检的推理模型 deepseek-r1）一致有效的唯一指标；general 词表中的互动尾巴、万能开场是当代特征，2023 语料测不到（诚实标注，不造数字）。完整校准表与被证伪删除的规则见 [docs/rules.md](docs/rules.md)。
