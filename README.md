...
human-vs-ai profiles                 # 列出场景（当前 academic，general 在路上）
```
...
- **按场景组织**：规则库不是一套大杂烩。实测证明词表规则的区分度强依赖文体（学术套话在问答语料上区分度≈0），所以每个 profile 一套词表 + 共用统计底盘
...
| 语料 | 指标 | AI | 真人 | AUROC |
|---|---|---|---|---|
| C-ReD 论文摘要（真人 80 vs 四模型 320） | 词表规则句均命中 | 0.185–0.338 | 0.053 | **0.803** |
| C-ReD 论文摘要 | 句长 CV | 0.275–0.392 | 0.491 | **0.798** |
| HC3-Chinese 问答（各 120） | TTR（MATTR） | 0.610 | 0.696 | 0.800 |

句长 CV 是对四个模型（含最难检的推理模型 deepseek-r1）一致有效的唯一指标。完整校准表与被证伪删除的规则见 [docs/rules.md](docs/rules.md)。
